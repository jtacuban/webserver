...
function property_photos_insert(&$error_message = '') {
	...
	// hook: property_photos_before_insert
	if(function_exists('property_photos_before_insert')) {
		$args = [];
		if(!property_photos_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('property_photos', backtick_keys_once($data), $error);
	if($error) {
		$error_message = $error;
		return false;
	}
	...
	update_calc_fields('property_photos', $recID, calculated_fields()['property_photos']);

	// hook: property_photos_after_insert
	if(function_exists('property_photos_after_insert')) {
		$res = sql("SELECT * FROM `property_photos` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args = [];
		if(!property_photos_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	// record owner is current user
	$recordOwner = getLoggedMemberID();
	set_record_owner('property_photos', $recID, $recordOwner);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID'))) property_photos_copy_children($recID, Request::val('SelectedID'));
...
function property_photos_update(&$selected_id, &$error_message = '') {
	...
	if(function_exists('property_photos_after_update')) {
		$res = sql("SELECT * FROM `property_photos` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
...
function property_photos_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	if($hasSelectedId) {
		$res = sql("SELECT * FROM `property_photos` WHERE `id`='" . makeSafe($selectedId) . "'", $eo);
