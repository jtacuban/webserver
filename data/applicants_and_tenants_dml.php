...
function applicants_and_tenants_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$childrenATag = '<a class="alert-link" href="applications_leases_view.php?filterer_tenants=' . urlencode($id[0]) . '">%s</a>';
	...
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="residence_and_rental_history_view.php?filterer_tenant=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
	...
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="employment_and_income_history_view.php?filterer_tenant=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
	...
	$rirow = db_fetch_row($rires);
	$childrenATag = '<a class="alert-link" href="references_view.php?filterer_tenant=' . urlencode($id[0]) . '">%s</a>';
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', sprintf($childrenATag, $rirow[0]), $RetMsg);
...
function applicants_and_tenants_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	if($hasSelectedId) {
		...
		$combo_driver_license_state->SelectedText = (isset($filterField[1]) && $filterField[1] == '8' && $filterOperator[1] == '<=>' ? $filterValue[1] : entitiesToUTF8(''));
		$combo_status->SelectedText = (isset($filterField[1]) && $filterField[1] == '13' && $filterOperator[1] == '<=>' ? $filterValue[1] : entitiesToUTF8('Applicant'));
	}
